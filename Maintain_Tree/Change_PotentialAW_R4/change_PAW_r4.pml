...
init {
  ...
  atomic{
    ...
    potentialAW_change(4,8,7);
  }
}

/*Verification for when R4 has as PAW root of R3 */
ltl ltl_test {(<>([](ROUTER_INTEREST(0)==in && ROUTER_INTEREST(1)==in && ROUTER_INTEREST(2)==ni && ROUTER_INTEREST(3)==in && UPSTREAM_INTEREST(3,7)==ui &&
  DOWNSTREAM_INTEREST(0,1)==di && DOWNSTREAM_INTEREST(0,2)==ndi && DOWNSTREAM_INTEREST(1,5)==di && DOWNSTREAM_INTEREST(2,6)==ndi &&
  INTERFACE_ASSERT_STATE(0,1)==aw && INTERFACE_ASSERT_STATE(0,2)==na && INTERFACE_ASSERT_STATE(1,5)==aw && INTERFACE_ASSERT_STATE(2,6)==na)))}
