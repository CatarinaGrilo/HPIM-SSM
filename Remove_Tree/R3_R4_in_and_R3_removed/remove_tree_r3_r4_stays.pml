...
init {
  ...
  atomic{
    interfaceFailure(3,7) 
  }
}

/*Verification for when the tree contains R3 and R4 and then only R3 is removed*/
ltl ltl_test {(<>([](ROUTER_INTEREST(0)==in && ROUTER_INTEREST(1)==ni && ROUTER_INTEREST(2)==in &&
  DOWNSTREAM_INTEREST(0,1)==ndi && DOWNSTREAM_INTEREST(0,2)==di && DOWNSTREAM_INTEREST(1,5)==ndi && DOWNSTREAM_INTEREST(2,6)==di &&
  INTERFACE_ASSERT_STATE(0,1)==na && INTERFACE_ASSERT_STATE(0,2)==aw && INTERFACE_ASSERT_STATE(1,5)==na && INTERFACE_ASSERT_STATE(2,6)==aw)))}
