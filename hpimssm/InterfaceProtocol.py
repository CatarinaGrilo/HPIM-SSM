...
from hpimssm.Interface import Interface
from hpimssm.Neighbor import Neighbor
from hpimssm.Packet.Packet import Packet
from hpimssm.Packet.ReceivedPacket import ReceivedPacket
from hpimssm.ReliableMsgTransmission import ReliableMessageTransmission
from hpimssm.tree.protocol_globals import MSG_FORMAT, HELLO_HOLD_TIME_TIMEOUT

from hpimssm.Neighbor import Synced

if MSG_FORMAT == "BINARY":
    from hpimssm.Packet.PacketProtocolHelloOptions import PacketNewProtocolHelloHoldtime as PacketProtocolHelloHoldtime, \
        PacketNewProtocolHelloCheckpointSN as PacketProtocolHelloCheckpointSN
    from hpimssm.Packet.PacketProtocolHello import PacketNewProtocolHello as PacketProtocolHello
    from hpimssm.Packet.PacketProtocolHeader import PacketNewProtocolHeader as PacketProtocolHeader
    from hpimssm.Packet.PacketProtocolSync import PacketNewProtocolSyncEntry as PacketProtocolHelloSyncEntry, \
    ...
    from hpimssm.Packet.PacketProtocolAck import PacketNewProtocolAck as PacketProtocolAck
else:
    from hpimssm.Packet.PacketProtocolHelloOptions import PacketProtocolHelloHoldtime, PacketProtocolHelloCheckpointSN
    from hpimssm.Packet.PacketProtocolHello import PacketProtocolHello
    from hpimssm.Packet.PacketProtocolHeader import PacketProtocolHeader
    from hpimssm.Packet.PacketProtocolSync import PacketProtocolHelloSync
    from hpimssm.Packet.PacketProtocolAssert import PacketProtocolAssert
    from hpimssm.Packet.PacketProtocolInterest import PacketProtocolPrune, PacketProtocolJoin
    from hpimssm.Packet.PacketProtocolSync import PacketProtocolHelloSyncEntry
    from hpimssm.Packet.PacketProtocolAck import PacketProtocolAck
...
class InterfaceProtocol(Interface):
    ...
    def get_tree_state(self, source_group):
        ...
        for n in list(self.neighbors.values()):
            neighbor_interest_state = n.get_tree_state(tree=source_group)
            print("NEIGHBOR INTEREST STATE:", neighbor_interest_state)
            if not interest_state and neighbor_interest_state:
                interest_state = neighbor_interest_state

        print("INTERFACE INTEREST STATE:", interest_state)
    ...
    def receive_hello(self, packet):
        ...
        if self.join:
            if (self.get_ip() == '10.0.0.4') and (ip == "10.0.0.1"):
                #time.sleep(10)
                #self.interface_logger.debug('Sending Join message - hardcoded')
                self.send_join('10.0.3.100', '224.12.12.12')
                #self.send_join('10.0.3.120', '224.12.12.12')
                #self.send_join('10.0.3.140', '224.12.12.12')
                #self.send_join('10.0.3.160', '224.12.12.12')
                #self.send_join('10.0.3.180', '224.12.12.12')
                self.join = False

        if self.prune:
            if (self.get_ip() == '10.0.0.4') and (ip == "10.0.0.3"):
                #time.sleep(15)
                #self.interface_logger.debug('Sending Prune message - hardcoded')
                self.send_prune('10.0.3.100', '224.12.12.12')
                #self.send_prune('10.0.3.120', '224.12.12.12')
                #self.send_prune('10.0.3.140', '224.12.12.12')
                #self.send_prune('10.0.3.160', '224.12.12.12')
                #self.send_prune('10.0.3.180', '224.12.12.12')
                self.prune = False
    ...
    def receive_assert(self, packet):
        ...
        """
        from hpimssm.tree.metric import AssertMetric
        neighbor_source_ip = packet.ip_header.ip_src
        boot_time = packet.payload.boot_time
        ...
        with self.neighbors_lock:
            ...
            try:
                if neighbor.recv_reliable_packet(sequence_number, source_group, boot_time):
                    ...
                    #self.neighbors_lock.release()
                    Main.kernel.recv_assert_msg(source_group, self, received_metric)
    ...
    def send_join(self, source, group):
        ...
        with self.sequencer_lock:
            with self.reliable_transmission_lock:
                self.get_reliable_message_transmission(tree).send_join(source, group)
    ...
    def send_prune(self, source, group):
        ...
        with self.sequencer_lock:
            with self.reliable_transmission_lock:
                self.get_reliable_message_transmission(tree).send_prune(source, group)
