...
from hpimssm.InterfaceIGMP import InterfaceIGMP
from hpimssm.InterfaceProtocol import InterfaceProtocol
from hpimssm.RWLock.RWLock import RWLockWrite
from hpimssm.tree import protocol_globals
from hpimssm.tree.KernelEntry import KernelEntry
...
class Kernel:
    ...
    def notify_unicast_changes(self, subnet):
        print("ENTROU NOTIFY UNICAST CHNAGES")
        with self.rwlock.genWlock():
        ...
        print("SAIU NOTIFY UNICAST CHANGES")

    def recv_interest_msg(self, source_group, interface: "InterfaceProtocol"):

        self.interface_logger.debug("ENTROU RCV INTEREST")
        ...
        with self.rwlock.genRlock():
            ...
            interest_state = interface.get_tree_state(source_group)

            if (ip_src not in self.routing) or (ip_dst not in self.routing.get(ip_src, {})):
                self.create_entry(ip_src, ip_dst)
                self.routing[ip_src][ip_dst].check_interface_state(interface.vif_index, interest_state)
        ...
        self.interface_logger.debug("SAIU RCV INTEREST")


    def recv_assert_msg(self, source_group, interface: "InterfaceProtocol", received_metric):
        ...
        with self.rwlock.genWlock():
            ...
            if (ip_src not in self.routing) or (ip_dst not in self.routing.get(ip_src, {})):
                self.create_entry(ip_src, ip_dst)
                self.routing[ip_src][ip_dst].recv_assert_msg(interface.vif_index, received_metric)
                self.routing[ip_src][ip_dst].check_interface_state(interface.vif_index, interest_state)
                ...
                self.interface_logger.debug('in recv_assert_msg_else')
                interface.remove_tree_state(ip_src, ip_dst)
    ...
    def create_entry(self, ip_src, ip_dst):
        ...
        print("IS DIRECTLY CONNECTED: ", is_directly_connected)
        ...
        if ip_src not in self.routing or ip_dst not in self.routing[ip_src]:
            ...
            for interface in self.protocol_interface.values():
                interest_state = interface.get_tree_state((ip_src, ip_dst))
            ...
            if ip_dst not in self.routing[ip_src]:
                self.routing[ip_src][ip_dst] = KernelEntry(ip_src, ip_dst, interest_state_dict)
    ...
    def snapshot_multicast_routing_table(self, vif_index):
        ...
        for (ip_src, src_dict) in self.routing.items():
            for (ip_dst, kernel_entry) in self.routing[ip_src].items():
                ...
                if tree is not None and tree:
                    trees_to_sync[(ip_src, ip_dst)] = 0x7FFFFFFF
    ...
    def recheck_all_trees(self, vif_index: int):
        ...
        with self.rwlock.genWlock():
            ...
            for tree in known_trees:
                if interface is not None:
                    interest_state = interface.get_tree_state(tree)
                else:
                ...
                if tree[0] not in self.routing or tree[1] not in self.routing.get(tree[0], {}):
                    ...
                    self.routing[tree[0]][tree[1]].check_interface_state(vif_index, interest_state)
        print("SAIU RECHECK")
